...
Nous avons vu précédemment comment vérifier qu’un utilisateur est connecté ou non pour autoriser l’accès à des routes. Nous avons également vu comment autoriser certaines routes en fonction du rôle (`RBAC`)

Il est important également de protéger les `Server Actions` (qui ne sont pas des routes). Une méthode simple consiste à vérifier que le `user` est `authentifier` (via la session par exemple).
...
Mais il est également possible de protéger certaines fonctionnalités en fonction du `role`. Exemple :
...
Pour simplifier l’exercice, ces formulaires sont fournis et connectés au `server action`.
...
- Les `ADMIN` et `SUPER_ADMIN` peuvent changer le rôle de n’importe quel `user` dans l’application (`changeUserRole`)
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20PRO&entry.1430994900=05.Next%20Auth&entry.533578441=12%20Authorization-server-action).
