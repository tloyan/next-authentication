...
Dans cet section nous verrons les sessions simple basées sur les cookies.
...
- La session est décodée / vérifiée pour extraire le `userId` coté le server
...
Dans cet exercice tu vas devoir créer le mécanisme de session gérer par cookies.
...
**🐶** Dans **`auth.ts`** créé une session après la création de compte ou la connexion et supprime le cookie sur le `logout`
...
- Utilise `verifySession()` et `getUserById` pour afficher : `Hello {user.email}` si un utilisateur est connecter
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20PRO&entry.1430994900=5.Next%20Auth&entry.533578441=04%20session-management-cookies).
