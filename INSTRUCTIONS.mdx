...
Dans l’exemple précédent, le cookie (contenant des informations sensibles comme le `userId`) était quasiment en clair dans le navigateur (URL encodé) :
...
- **🐶 Crée** `getConnectedUser` dans la page de `auth` (plus tard nous le déplacerons dans le dossier lib)

<aside>
💡 À noter que le `cache` fonctionne par requête HTTP.
...
Quand on regarde les `log` de `console.log('Page : user', user)` on peut voir l’objet `user` complet (avec le `password`/`salt` et autres informations sensibles)

Pour éviter d’exposer dans données sensibles, nous pouvons ajouter une couche qui vient nettoyer certaines données : **`DAL` (`Data Acces Layer`)**

![dal.png](/course/dal.png)

Le principe est une fonction qui prend une entrée une donnée (venant de la BDD par exemple) et retourner une donnée allégée, exemple :
...
- **🐶 Crée ce `DTO` et retourne un `DTO` dans `getConnectedUser`**
...
2. **`taintUniqueValue`** :
   ...
   experimental_taintUniqueValue(
     'Do not pass the API key to the client.',
     globalThis,
     apiKey
   )
   ```
...
- **🐶 Modifie le `DTO` en ajoutant `experimental_taintUniqueValue` pour le champ `password` de l’objet user.**
...
PS : `getConnectedUser` et `userDTO` seront déplacés ensuite dans un fichier `dal.ts` (`data access layer`)
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20PRO&entry.1430994900=05.Next%20Auth&entry.533578441=05%20Session%20-%20JWT%20-%20DTO).
