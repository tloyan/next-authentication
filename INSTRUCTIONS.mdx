...
Jusqu'à la nous avons mise en place l’authentification, nous allons maintenant faire de l’autorisation avec un cas relativement basique. La protection de routes en fonction des rôles. Jusque là nous avions :

- Non authentifier → routes publiques
- Authentifier → routes protégées

Nous n’avons pas assez de finesse pour gérer d’autres type de routes comme par exemple celles d’un administrateur. Pour faire cela nous allons introduire des ROLES
...
Il va falloir ce baser sur ces rôles pour gérer les accès.

### Accès Database dans le middleware ?

Dans le middleware nous accédons a la session stocké dans le cookies

```tsx
const hasSession = session?.userId || session?.sessionId
```

On pourrait envisager faire un appelle à cette fonction pour récupérer le rôle
...
- Le middleware s’exécute dans le runtime [EDGE](https://vercel.com/docs/functions/runtimes/edge-runtime) (qui n’a pas accès toutes l’api de Node, comme par exemple FS) Il nous est donc impossible d’appeler notre BDD.
- On pourrait utiliser d’autre système compatible (`unstorage`, `drizzle`) mais il ne faut pas oublier que le `midleware` est exécuter à chaque requête serveur. Faire des appels en BDD ralentirait considérablement le site.

**La solution ?**

- Mettre l’information rôle dans la session (cookie)
- Il sera peu couteux de récupérer cette information dans le middleware

**Est ce sécure ?**

- Un utilisateur malveillant pourrait envoyer une requête en changeant le role (USER→ ADMIN) dans la requête HTTP.
- Pour éviter cela, **IL EST IMPERATIF d’utiliser JWT. Avec** JWT une requête modifiée serait invalidée coté serveur.
...
- Si un utilisateur a le rôle `REDACTOR` ou `MODERATOR` : il peut accéder à tout plus : `/exercices/ban`
...
**🐶 Implémente les règles demandé par le chef de projet.**
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20PRO&entry.1430994900=5.Next%20Auth&entry.533578441=11-middleware-authorization).
