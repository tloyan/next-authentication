...
Nous avons développé deux stratégies pour la gestion des sessions :
...
L’idée est que les couches qui appellent les fonctions de sessions n’ont pas à se préoccuper du type de gestion de session.
...
Et le type de session sera défini en variable d’environnement
...
Pour les besoins de l’exercice, nous avons modifié tous les imports de sessions dans `auth.ts et dal.ts` et nous avons maintenant un fichier `sessions.ts` qui appelle la version `stateless`
...
- **🐶 Implémente le `stategy pattern` pour pouvoir switcher d’implémentation en fonction de** `SESSION_TYPE` défini dans le `.env`
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20PRO&entry.1430994900=05.Next%20Auth&entry.533578441=09%20Stategy%20Pattern).
